...
//
// hatrapframeterrupt, exception, or system call from user space.
...
usertrap(void)
{
  ...
  if(which_dev == 2){
    if (p->alarminterval>0 && !p->alarmhandleractive){
      // cputimeused, alarminterval, alarmhandler
      if (p->cputimeused > p->alarminterval){
        ualarmfrsave();
